...
def combien_de_Coul_sur_une_diagonale(echiquier, case, No_diag=1, couleur='B') : # Diagonale 1 : gauche-droite , 2: droite-gauche
   ...
   temp_lst=[1 for (l,c) in Liste if l==c] if No_diag==1 else [1 for (l,c) in Liste if l+c==4]
   return sum(temp_lst)
...
def valeur_d_une_case(echiquier, couleur, case) :
   ...
   ND1_1 = combien_de_Coul_sur_une_diagonale(echiquier, case, 1, couleur) # 0 si on n'est pas sur la première diagonale
   ND1_2 = combien_de_Coul_sur_une_diagonale(echiquier, case, 1, Adversaire) # 0 si on n'est pas sur la première diagonale
   #print(f"Pour la case {case}, ND1_1, ND1_2 ", ND1_1, ND1_2)
   Val_diagonale_1 = ND1_1 - ND1_2
 
   ND2_1 = combien_de_Coul_sur_une_diagonale(echiquier, case, 2, couleur) # 0 si on n'est pas sur la première diagonale
   ND2_2 = combien_de_Coul_sur_une_diagonale(echiquier, case, 2, Adversaire) # 0 si on n'est pas sur la première diagonale
...
if __name__=="__main__" :
   ...
   # =========== EXEMPLE 3 ==================
   LB=[(1,2),(2,2)];  LN=[(1,1),(2,1)] ; LV=[(1,3), (2,3) , (3,1), (3,2), (3,3)]
   echiquier=[LB,LN,LV]
   ...
   # =========== EXEMPLE 3 ==================
   LB=[(1,2),(2,2), (3,3)];  LN=[(1,1),(2,1), (2,3)] ; LV=[(1,3),  (3,1), (3,2)]
   echiquier=[LB,LN,LV]
   print("Les Blancs = ", LB, "Les Noirs = ", LB, "\nLes Vides = ", LV, "\n")
   ...
   print(f"Pour la couleur {couleur}, on a trouvé la liste des valeurs : \n{dico_valeurs_pour_couleur}")

   liste_cles_du_dico_trie= sorted(dico_valeurs_pour_couleur, key=dico_valeurs_pour_couleur.get, reverse=True)
   print(f"le meilleur coup à jouer pour la couleur {couleur} = {liste_cles_du_dico_trie[0]} dont la valeur ={dico_valeurs_pour_couleur[liste_cles_du_dico_trie[0]]}")
   print('-'*60)
   couleur='N'
   dico_valeurs_pour_couleur={case : valeur_d_une_case(echiquier, couleur, case) for case in LV}
   print(f"Pour la couleur {couleur}, on a trouvé la liste des valeurs : \n{dico_valeurs_pour_couleur}")
